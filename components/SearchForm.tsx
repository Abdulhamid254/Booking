...
export const formSchema = z.object({
    location:z.string().min(2,"Must contain atleast 2 characters or more").max(10),
    dates: z.object({
        from: z.date(),
        to: z.date(),
    }),
    ...
    children: z.string().min(0).max(12, {
        message:'Max 12 chikdren Occupancy'
    }),
    rooms: z.string().min(1, {
        message: "Please select a least 1 rooom",
    }),
});
...
export default function SearchForm () {
  ...
  function onSubmit(values:z.infer<typeof formSchema>) {
    // adding the details to url
    const checkin_monthday = values.dates.from.getDate().toString();
    const checkin_month = (values.dates.from.getMonth() + 1).toString();
    const checkin_year = values.dates.from.getFullYear().toString();
    const checkout_monthday = values.dates.to.getDate().toString();
    const ccheckout_month = (values.dates.to.getMonth() + 1).toString();
    const checkout_year = values.dates.to.getFullYear().toString();

    const checkin = `${checkin_year}-${checkin_month}-${checkin_monthday}`;
    const checkout = `${checkout_year}-${ccheckout_month}-${checkout_monthday}`;
                    ...
                    <FormItem className="flex flex-col">
                        ...
                        <Popover>
                            <PopoverTrigger asChild>
                                <FormControl>
                                    <Button id="date" name="dates" variant={"outline"}
                                    className={cn("w-full lg:w-[300px] justify-start text-left font-normal",
